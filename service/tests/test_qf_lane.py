...
def test_the_open_equity_valley_is_marked_in_a_qf_only_run():
    """H1, 2026-08-20. The valley was marked inside the HalfTrend section,
    BELOW the `--strategy qf` short-circuit, so every qf-only run reported a
    fabricated `max open-equity valley 0.00` -- across 118 trades on the
...
def test_the_valley_is_never_shallower_than_the_closed_drawdown():
    """The account was at least as deep under water intrabar as it was at
    any close. True in every mode -- and only true in `both` if QuickFlip's
