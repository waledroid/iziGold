"""Contract tests for the dashboard control endpoints (/ui/mode,
/ui/proposal/{pid}, /ui/close-all) and the extended /ui/state."""
import importlib
...
def test_mode_set_and_reflected_in_state(client):
    r = client.post("/ui/mode", json={"mode": "auto"})
    assert r.status_code == 200 and r.json() == {"mode": "auto"}
    assert client.get("/ui/state").json()["mode"] == "auto"
    r = client.post("/ui/mode", json={"mode": "manual"})
    assert r.status_code == 200
    assert client.get("/ui/state").json()["mode"] == "manual"


def test_mode_invalid_rejected(client):
    assert client.post("/ui/mode", json={"mode": "yolo"}).status_code == 400
    assert client.post("/ui/mode", json={}).status_code == 400
...
def test_proposal_take_and_skip(client):
    ...
    r = client.post(f"/ui/proposal/{pid}", json={"action": "take"})
    ...
    r = client.post(f"/ui/proposal/{pid2}", json={"action": "skip"})
...
def test_proposal_bad_input(client):
    db = _db(client)
    pid = db.create_proposal("entry", "BUY", "s", 2400.0, None)
    assert client.post(f"/ui/proposal/{pid}", json={"action": "yes"}).status_code == 400
    assert client.post("/ui/proposal/999999", json={"action": "take"}).status_code == 404


# ------------------------------------------------------- /ui/close-all
def test_close_all_creates_approved_exit(client):
    r = client.post("/ui/close-all")
...
def test_close_all_conflict_while_in_flight(client):
    assert client.post("/ui/close-all").status_code == 200
    assert client.post("/ui/close-all").status_code == 409


def test_close_all_dispatched_via_heartbeat(client):
    pid = client.post("/ui/close-all").json()["proposal_id"]
